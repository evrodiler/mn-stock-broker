...
class SymbolsControllerTest {
    ...
    void symbolsEndpointReturnsTheCorrectSymbol() {
        ...
        var response = client.toBlocking().exchange("/" + testSymbol.value(), Symbol.class);  //localhost:8080/symbols/TEST
        assertEquals(HttpStatus.OK, response.getStatus());
    ...
    // filter and offset parameters
    {
        ...
        var max2_offset7 = client.toBlocking().exchange("/filter?max=2&offset=7", JsonNode.class);
        assertEquals(HttpStatus.OK, max2_offset7.getStatus());
        LOG.debug("Offset: 7 {}",max2_offset7.getBody().get().toPrettyString());
        assertEquals(2, max2_offset7.getBody().get().size());
